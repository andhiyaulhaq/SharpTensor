  ...
  renderImageList(images) {
    ...
    this.dom.imageList.querySelectorAll('.image-item').forEach((item) => {
      item.addEventListener('click', () =>
        state.set({ currentImageIndex: parseInt(item.dataset.index) })
      );
    });
  }
  ...
  async confirmMagicMask() {
    ...
    if (isSegTask) {
      ...
      polygon = ContourTracer.trace(
        activeMask,
        state.data.currentImageBitmap.width,
        state.data.currentImageBitmap.height
      );
