...
class App {
  ...
  async handleTryDemo(): Promise<void> {
    try {
      ...
      // Start the guided tour immediately now that animations are removed
      this.startTour();
    } catch (err) {
  ...
  advanceTour(step: 'step2-interact' | 'complete'): void {
    ...
    if (step === 'step2-interact') {
      ...
      this.updateStatus('Tour complete! Feel free to open your own local folder.');
    }
  }
  ...
  private renderTourTooltip(targetEl: HTMLElement, text: string): void {
    ...
    // If it's in the right sidebar (x > viewport width / 2)
    let left = 0;
    let top = 0;
  ...
  async handleAutoLabelDataset(): Promise<void> {
    if (state.data.tourActive && state.data.tourStep === 'step1-autolabel') {
      ...
      this.advanceTour('step2-interact');
      return;
    }
