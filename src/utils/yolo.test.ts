...
import { YoloHelper } from './yolo';
import { BoundingBox } from '../core/types';

describe('YoloHelper', () => {
  describe('toYolo', () => {
    it('should convert pixel coordinates to YOLO format', () => {
      const box: BoundingBox = { classId: 1, x: 100, y: 100, width: 200, height: 200, id: 1 };
      const imgWidth = 800;
      const imgHeight = 800;
  ...
  describe('Colors', () => {
    it('withAlpha should inject alpha', () => {
      ...
      expect(YoloHelper.withAlpha('#ff0000', 0.5)).toBe('#ff000080');
      expect(YoloHelper.withAlpha(null as any)).toBe('rgba(255, 255, 255, 0.25)');
      expect(YoloHelper.withAlpha('rgb(255, 255, 255)', 0.5)).toBe('rgb(255, 255, 255)');
    });
    ...
    it('getContrastColor should return black or white', () => {
      ...
      expect(YoloHelper.getContrastColor('#fff')).toBe('#000000');
      expect(YoloHelper.getContrastColor('hsl(0, 100%, 90%)')).toBe('#000000');
      expect(YoloHelper.getContrastColor('hsl(0, 100%, 10%)')).toBe('#ffffff');
      expect(YoloHelper.getContrastColor('rgb(255, 0, 0)')).toBe('#ffffff');
      expect(YoloHelper.getContrastColor(null as any)).toBe('#ffffff');
    });
  });
});
